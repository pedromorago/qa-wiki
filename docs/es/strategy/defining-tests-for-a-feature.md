...
Definidos los casos, cada uno se asigna a la **capa más baja donde se pueda verificar con confianza** ([pirámide](/es/fundamentals/the-testing-pyramid)):

- ¿Es lógica pura? → unit (probablemente ya la cubre el developer).
- ¿Es comportamiento de API o persistencia? → integración/API.
- ¿Es algo que el usuario ve o un flujo que cruza el sistema? → E2E, con moderación ([qué merece un E2E](/es/strategy/what-e2e-tests-should-cover)).

Las capas concretas, con ejemplos: [backend](/es/strategy/backend-testing-layers) y [frontend](/es/strategy/frontend-testing-layers).
