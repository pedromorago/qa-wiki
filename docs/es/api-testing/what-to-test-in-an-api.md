...
Aquí es donde se aplican [particiones y valores límite](/es/fundamentals/test-case-design), igual que en UI:
...
- Campos extra no esperados en el body (p. ej. enviar `status: "active"` o `monthlyPrice: 0` al crear un pedido de servicio): ¿los ignora o los persiste? (*mass assignment*).
...
- El clásico de los clásicos: **IDOR** — con el token del cliente A, pedir `GET /customers/B/service-orders`. ¿Puedo ver los pedidos de otro cliente?
...
- `POST` duplicado (doble clic, reintento de red): ¿crea dos pedidos de servicio?
