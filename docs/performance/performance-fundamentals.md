...
- **Latency by percentiles**: p50 (median), p95, p99. **The average lies**: a 200 ms average can hide a 4-second p99, and that p99 is the slowest 1% of requests — and since each user makes many requests, most users end up hitting it sooner or later.
...
The tool matters less than the load model and the metrics: the test types, metrics and design rules above apply the same in all three.
...
- **Generating load from your laptop** against a remote environment: you end up measuring your Wi-Fi.
- **Ignoring warm-up**: the first minutes (cold caches, JIT) aren't representative.
- **Not repeating the test**: a single run is an anecdote, not evidence.
