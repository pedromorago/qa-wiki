...
A failing E2E test tells you "something is wrong in the ordering flow". A failing unit test tells you "the `calculateMonthlyFee` function returns the wrong monthly fee for the tariff". The lower down you catch the problem, the cheaper it is to diagnose.
...
- E2E tests are reserved for the **critical business flows** (mostly happy paths, plus the errors that block the user): customer sign-up, contracting a product, service activation.
