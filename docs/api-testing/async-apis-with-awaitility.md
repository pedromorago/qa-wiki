...
public boolean isOperationSuccess(String operationId) {
    Awaitility.await()
            .atMost(5, TimeUnit.SECONDS)       // max timeout → ConditionTimeoutException
...
- The same concept exists in other stacks: `expect.poll` / `expect(...).toPass()` in Playwright, `cy.waitUntil` (the `cypress-wait-until` plugin) in Cypress, `tenacity` in Python.
